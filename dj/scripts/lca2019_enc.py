...
LINEPOS_Y = {
            5: ['181.51', '253.00', '324.49', '395.98', '467.47'],
                4: ['217.26', '288.75', '360.24', '431.73'],
                    3: ['253.00', '324.49', '395.98'],
                        2: ['288.75', '360.24'],
                            1: ['324.49'],

                            }
LINEPOS_X = '623.81'
TITLE_LINE_LENGTH = 28

TITLE_LINE_STYLE = ("font-style:normal;font-variant:normal;font-weight:normal;"
    "font-stretch:normal;font-size:59.9992485px;line-height:100%;font-family:"
    "'Space Mono';-inkscape-font-specification:'Space Mono, Normal';font-"
    "variant-ligatures:normal;font-variant-caps:normal;font-variant-numeric:"
    "normal;font-feature-settings:normal;text-align:center;writing-mode:lr-tb;"
    "text-anchor:middle")

def make_tspan(data):
    i, (pos, line) = data
    name = 'title%s' % (i+1)
    attrs = {'y': pos, 'x': LINEPOS_X, 'style': TITLE_LINE_STYLE, 'id': name}
    elem = xml.etree.ElementTree.Element('ns0:tspan', attrs)
    elem.text = line
    return elem

# Integrate this function just before you return `texts` in line 180
def get_title(texts):
    main_title = wrap(texts.pop('title'), TITLE_LINE_LENGTH, break_long_words=False)
    subtitle = wrap(texts.pop('title2'), TITLE_LINE_LENGTH, break_long_words=False)
    texts['title'] = main_title + subtitle
    return texts


# Integrate the content of this function in your own mk_title_svg
def mk_title_svg(tree):
    for key in texts:
        if key in tree[1]:
            if key == 'title':
                # This should map to a <text> tag
                title = texts['title']
                line_positions = LINEPOS_Y[len(title)]

                # Set text box position to first line
                tree[1][key].set('y', line_positions[0])

                lines = map(make_tspan, enumerate(zip(line_positions, title)))
                tree[1][key].extend(list(lines))
...
class enc(process):
    ...
    def get_title_text(self, episode):
        ...
        # '' to padd unused slots
        # import pdb; pdb.set_trace()
        authors = [ author.strip() for author in authors.split(', ')]
        ats = zip(authors, episode.twitter_id.split(', '))
        authors = []
        for a,t in ats:
            if t:
                author = "{} | {}".format(a,t)
        ...
        author0, author1, author2 = (authors+['',''])[:3]
