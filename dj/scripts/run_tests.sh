...
if [ ! -e $DIR/$HOUR:00:00.dv ]; then
 ...
 melt -profile dv_ntsc t3.txt out=30 -consumer avformat:$DV_DIR/$HOUR:03:00.dv pix_fmt=yuv411p
 melt -profile dv_ntsc t4.txt out=180 -consumer avformat:$DV_DIR/$HOUR:04:58.dv pix_fmt=yuv411p
 melt -profile dv_ntsc t5.txt out=30 -consumer avformat:$DV_DIR/$HOUR:06:00.dv pix_fmt=yuv411p
 rm t?.txt
fi
